...
window.obtenerBultos = function () {
    var id = $("#id").val();
    $.ajax({url: "/mobile/get/obtener-bultos", dataType: "json", timeout: 10000, data: {id: id}, type: "GET",
        success: function (res) {
            if (res.success === true) {
                if (res.result.length > 0) {
                    var html = '';
                    for (var i = 0; i < res.result.length; i++) {
                        ...
                        html += "<tr>" + 
                                "<td>" + row['numBulto'] + "</td>" +
                                "<td>" + ((row['uuid'] !== null) ? row['uuid'] : '') + "</td>" +
                                "<td><a href=\"/mobile/bodega/editar-bulto?id=" + row['id'] + "\"><i class=\"fas fa-pencil-alt\"></i></a></td>" + 
                                "</tr>";
                    }
