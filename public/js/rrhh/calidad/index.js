...
function eliminarDirectorio(id) {
    $.post("/rrhh/calidad/comprobar-contenido", {id: id})
            .done(function (res) {
                if (res.success === true) {
                    $.alert({
                        title: "¡Advertencia!",
                    ...
                    $.post("/rrhh/calidad/eliminar-directorio", {id: id})
                        .done(function (res) {
                            if (res.success === true) {
        ...
        buttons: {
            si: {
                ...
                action: function () {
                    var ids = [];
                    ids.push(id);
                    $.post("/rrhh/calidad/eliminar-archivos", {ids: ids})
                            .done(function (res) {
                                if (res.success === true) {
                                    window.location.href = window.location.pathname + ((queryString.directorio !== undefined) ? "?directorio=" + queryString.directorio : "");
                                }
                ...
                action: function () {
                    if ($("#editFileForm").valid()) {
                        $("#editFileForm").ajaxSubmit({
                            cache: false,
                            url: "/rrhh/calidad/editar-archivo",
                            type: "post",
                            dataType: "json",
                ...
                action: function () {
                    if ($("#editFolderForm").valid()) {
                        $("#editFolderForm").ajaxSubmit({
                            cache: false,
                            url: "/rrhh/calidad/editar-directorio",
                            type: "post",
                            dataType: "json",
                            success: function (res) {
                                if (res.success === true) {
                    ...
                    action: function () {
                        if ($("#uploadForm").valid()) {
                            $("#uploadForm").ajaxSubmit({
                                cache: false,
                                url: "/rrhh/calidad/subir-archivos",
                                type: "post",
                                dataType: "json",
                                success: function (res) {
                                    if (res.success === true) {
                    ...
                    action: function () {
                        if ($("#folderForm").valid()) {
                            $("#folderForm").ajaxSubmit({
                                cache: false,
                                url: "/rrhh/calidad/crear-directorio",
                                type: "post",
                                dataType: "json",
                                success: function (res) {
                                    if (res.success === true) {
