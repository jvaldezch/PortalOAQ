...
$(document).ready(function () {
    ...
    dg.edatagrid({
        ...
        columns: [
            [
                ...
                {field: "nombre", width: 250, title: "Cliente"},
                {field: "acta", width: 90, title: "Acta"},
                {field: "multa", width: 120, title: "Multa"},
                {field: "pagada", width: 60, title: "Pagada", formatter: function (value, row) {
                        if (parseInt(value) === 1) {
                            return "Si";
                        } else {
                            return "No";
    ...
    $(document.body).on("click", ".fa-pencil-alt", function (ev) {
        ev.preventDefault();
        var id = $(this).data("id");
        $.confirm({ title: "Editar incidencia", escapeKey: "cerrar", boxWidth: "550px", useBootstrap: false, type: "green",
            buttons: {
                guardar: {btnClass: "btn-green", action: function () {
                    if ($("#form_incidence").valid()) {
                        $("#form_incidence").ajaxSubmit({url: "/operaciones/reportes/actualizar-incidencia", dataType: "json", type: "POST",
        ...
        $.confirm({ title: "<i class=\"fas fa-exclamation-triangle\"></i> Confirmar", escapeKey: "cerrar", boxWidth: "350px", useBootstrap: false, type: "red",
            buttons: {
                si: {btnClass: "btn-red", action: function () {
                    $.ajax({url: '/operaciones/reportes/borrar-incidencia', dataType: "json", type: "POST",
                        data: {id :id},
                        success: function (res) {
                            if (res.success === true) {
                                dg.edatagrid("reload");
    ...
    $(document.body).on("click", "#nuevaIncidencia", function (ev) {
        ev.preventDefault();
        $.confirm({ title: "Nueva incidencia", escapeKey: "cerrar", boxWidth: "550px", useBootstrap: false, type: "green",
            buttons: {
                agregar: {btnClass: "btn-green", action: function () {
                    if ($("#form_incidence").valid()) {
                        $("#form_incidence").ajaxSubmit({url: "/operaciones/reportes/agregar-incidencia", dataType: "json", type: "POST",
                            success: function (res) {
                                if (res.success === true) {
                                    dg.edatagrid("reload");
                                } else {
    ...
    $.each(['edit', 'trash', 'patente', 'aduana', 'pedimento', 'referencia', 'cliente', 'fecha', 'comentarios', 'observaciones', 'acta', 'multa', 'pagada', 'responsable'], function (index, value) {
