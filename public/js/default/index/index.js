$(document).ready(function () {
    ...
    $("body").css({'background-image': 'url(/images/front/' + images[Math.floor(Math.random() * images.length)] + ')'});
    ...
    $(document.body).on("click", "#submit", function (e) {
        if ($("#formLogin").valid()) {
            $("#formLogin :input").attr("readonly", "true")
                    .addClass("disabled");
            $("#formLogin #submit").prop("disabled", true);
            $("#formLogin").ajaxSubmit({
                cache: false,
                url: "/default/ajax/login",
                type: "post",
                dataType: "json",
                timeout: 3000,
                success: function (res) {
                    if (res.success === true) {
                        ...
                        if (res.password) {
                            $("#password").before("<span class=\"error\">" + res.password + "</span>");
                        }
                        $("#formLogin :input").removeProp("readonly")
                                .removeClass("disabled");
                        $("#formLogin #submit").removeProp("disabled");
                    }
                }
            });
        }
    ...
    if (mobileCheck() === true) {
        window.location.href = "/mobile/main/index";
    }
