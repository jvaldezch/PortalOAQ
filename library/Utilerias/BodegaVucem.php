...
class Utilerias_BodegaVucem {
    ...
    function set_idBodega($_idBodega) {
    ...
    function __construct($cove = false, $edoc = false) {
        ...
        if (APPLICATION_ENV == "production") {
            $this->_misc->set_baseDir($this->_appconfig->getParam("expdest"));
        } else {
            $this->_misc->set_baseDir("D:\\xampp\\tmp\\expedientes");
        }
    ...
    protected function _guardarArchivoXml() {
        if ($this->_idBodega) {
            ...
            $b = $mpr->obtener($this->_idBodega);
            
            $directory = $this->_misc->directorioExpedienteDigitalBodega($b['siglas'], $this->_misc->trimUpper($this->_referencia));
    ...
    protected function _cleanFilename($filename) {
        return $this->_misc->formatFilename($filename);
    }
    ...
    public function getXmlFilename() {
        if ($this->_idBodega) {
            ...
            if (isset($this->_referencia)) {
                return $directory . DIRECTORY_SEPARATOR . $this->_cleanFilename('COVE_' . $this->_referencia . '_' . $this->_numeroFacturaOriginal . '.xml');
            } else {
                return $directory . DIRECTORY_SEPARATOR . 'COVE_' . sha1(microtime()) . '.xml';
            }
        }
    }
