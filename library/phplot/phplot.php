...
/////////////////////////////////////////////
    ...
    function SetRGBColor($color_asked, $alpha = 0) {
        if (empty($color_asked)) {
            ...
            $ret_val = $color_asked;
        } elseif (preg_match('/^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})?$/i', $color_asked, $ss)) {
        ...
        } elseif (preg_match('/(.+):([\d]+)$/', $color_asked, $ss) && isset($this->rgb_array[$ss[1]])) {
...
/////////////////////////////////////////////
    ...
    protected function SetDefaultFonts() {
        if ($this->use_ttf) {
            ...
            return $this->SetFontTTF('generic', '', 8) && $this->SetFontTTF('title', '', 14) && $this->SetFontTTF('legend', '', 8) && $this->SetFontTTF('x_label', '', 6) && $this->SetFontTTF('y_label', '', 6) && $this->SetFontTTF('x_title', '', 10) && $this->SetFontTTF('y_title', '', 10);
        }
        // Defaults for use of GD fonts:
        return $this->SetFontGD('generic', 2) && $this->SetFontGD('title', 5) && $this->SetFontGD('legend', 2) && $this->SetFontGD('x_label', 1) && $this->SetFontGD('y_label', 1) && $this->SetFontGD('x_title', 3) && $this->SetFontGD('y_title', 3);
    }
...
// Preferred usage for $font_id is a text element name (see SetFont()), but for compatibility
    ...
    function DrawText($which_font, $which_angle, $which_xpos, $which_ypos, $which_color, $which_text, $which_halign = 'left', $which_valign = 'bottom') {
        return $this->ProcessText(TRUE, $which_font, $which_angle, $which_xpos, $which_ypos, $which_color, $which_text, $which_halign, $which_valign);
    }
    ...
    function SizeText($which_font, $which_angle, $which_text) {
        ...
        return $this->ProcessText(FALSE, $which_font, $which_angle, 0, 0, 1, $which_text, '', '');
    }
...
/////////////////////////////////////////////
    ...
    function SetXDataLabelPos($which_xdlp) {
        $which_xdlp = $this->CheckOption($which_xdlp, 'plotdown, plotup, both, none, plotin, plotstack', __FUNCTION__);
        if (!$which_xdlp)
            return FALSE;
    ...
    function SetYDataLabelPos($which_ydlp) {
        $which_ydlp = $this->CheckOption($which_ydlp, 'plotleft, plotright, both, none, plotin, plotstack', __FUNCTION__);
        if (!$which_ydlp)
            return FALSE;
    ...
    function SetXTickLabelPos($which_xtlp) {
        $which_xtlp = $this->CheckOption($which_xtlp, 'plotdown, plotup, both, xaxis, none', __FUNCTION__);
        if (!$which_xtlp)
            return FALSE;
    ...
    function SetYTickLabelPos($which_ytlp) {
        $which_ytlp = $this->CheckOption($which_ytlp, 'plotleft, plotright, both, yaxis, none', __FUNCTION__);
        if (!$which_ytlp)
            return FALSE;
    ...
    function SetPieLabelType() {  // Variable arguments: $source, $type, ....
        ...
        if (empty($source)) {
            ...
            $this->pie_label_source = $this->CheckOptionArray($source, 'percent, value, label, index', __FUNCTION__);
...
/////////////////////////////////////////////
    ...
    function SetPlotBorderType($pbt) {
        $this->plot_border_type = $this->CheckOptionArray($pbt, 'left, right, top, bottom, sides, none, full', __FUNCTION__);
...
//////////////////////////////////////////////////////////
    ...
    protected function CalcMargins($maximize) {
        ...
        // Above the plot:
        $tick_labels_above = ($this->x_tick_label_pos == 'plotup' || $this->x_tick_label_pos == 'both' || ($this->x_tick_label_pos == 'xaxis' && $x_axis_pos == 'top'));
        ...
        // Below the plot:
        $tick_labels_below = ($this->x_tick_label_pos == 'plotdown' || $this->x_tick_label_pos == 'both' || ($this->x_tick_label_pos == 'xaxis' && $x_axis_pos == 'bottom'));
        ...
        // Left of the plot:
        $tick_labels_left = ($this->y_tick_label_pos == 'plotleft' || $this->y_tick_label_pos == 'both' || ($this->y_tick_label_pos == 'yaxis' && $y_axis_pos == 'left'));
        ...
        // Right of the plot:
        $tick_labels_right = ($this->y_tick_label_pos == 'plotright' || $this->y_tick_label_pos == 'both' || ($this->y_tick_label_pos == 'yaxis' && $y_axis_pos == 'right'));
        ...
        if ($this->x_tick_pos == 'plotup' || $this->x_tick_pos == 'both' || ($this->x_tick_pos == 'xaxis' && $x_axis_pos == 'top')) {
        ...
        if ($this->x_tick_pos == 'plotdown' || $this->x_tick_pos == 'both' || ($this->x_tick_pos == 'xaxis' && $x_axis_pos == 'bottom')) {
        ...
        if ($this->y_tick_pos == 'plotleft' || $this->y_tick_pos == 'both' || ($this->y_tick_pos == 'yaxis' && $y_axis_pos == 'left')) {
        ...
        if ($this->y_tick_pos == 'plotright' || $this->y_tick_pos == 'both' || ($this->y_tick_pos == 'yaxis' && $y_axis_pos == 'right')) {
...
// The plot type can customize how an end adjustment is applied:
    ...
    protected function CalcPlotRange($which) {
        ...
        list($plot_min, $adjust_min) = $this->CalcRangeInit($this->{"plot_min_$which"}, $implied, $this->{"min_$which"});
        list($plot_max, $adjust_max) = $this->CalcRangeInit($this->{"plot_max_$which"}, $implied, $this->{"max_$which"});
    ...
    protected function CalcTicks($which) {
        ...
        if ($this->GetCallback('debug_scale'))
            $this->DoCallback('debug_scale', __FUNCTION__, compact('which', 'tick_start', 'tick_end', 'tick_step'));
...
// X Label position fixups, for x_data_label_pos and x_tick_label_pos:
    ...
    protected function TuneAutoTicks($which, $min_ticks, $tick_mode, $tick_inc_integer) {
        ...
        if (isset($tick_mode)) {
            $tick_mode = $this->CheckOption($tick_mode, 'decimal, binary, date', 'Tune' . strtoupper($which) . 'AutoTicks');
            if (!$tick_mode)
                return FALSE;
...
/////////////////////////////////////////////
    ...
    protected function tile_img($file, $xorig, $yorig, $width, $height, $mode) {
        ...
        if ($mode == 'scale') {
            imagecopyresampled($this->img, $im, $xorig, $yorig, 0, 0, $width, $height, $tile_width, $tile_height);
            return TRUE;
        }
    ...
    protected function DrawXTitle() {
        ...
        if ($this->x_title_pos == 'plotup' || $this->x_title_pos == 'both') {
            ...
            $this->DrawText('x_title', 0, $xpos, $ypos, $this->ndx_x_title_color, $this->x_title_txt, 'center', 'bottom');
        }
        // Lower title
        if ($this->x_title_pos == 'plotdown' || $this->x_title_pos == 'both') {
            $ypos = $this->plot_area[3] + $this->x_title_bot_offset;
            $this->DrawText('x_title', 0, $xpos, $ypos, $this->ndx_x_title_color, $this->x_title_txt, 'center', 'top');
        }
        return TRUE;
    }
    ...
    protected function DrawYTitle() {
        ...
        if ($this->y_title_pos == 'plotleft' || $this->y_title_pos == 'both') {
            ...
            $this->DrawText('y_title', 90, $xpos, $ypos, $this->ndx_y_title_color, $this->y_title_txt, 'right', 'center');
        }
        if ($this->y_title_pos == 'plotright' || $this->y_title_pos == 'both') {
            $xpos = $this->plot_area[2] + $this->y_title_right_offset;
            $this->DrawText('y_title', 90, $xpos, $ypos, $this->ndx_y_title_color, $this->y_title_txt, 'left', 'center');
        }
    ...
    protected function DrawYAxis() {
        ...
        if (!$this->suppress_y_axis) {
            ImageLine($this->img, $this->y_axis_x_pixels, $this->plot_area[1], $this->y_axis_x_pixels, $this->plot_area[3], $this->ndx_grid_color);
        }
        return TRUE;
    }
    ...
    protected function DrawXTick($x, $x_pixels) {
        // Ticks on X axis
        if ($this->x_tick_pos == 'xaxis') {
            ImageLine($this->img, $x_pixels, $this->x_axis_y_pixels - $this->x_tick_cross, $x_pixels, $this->x_axis_y_pixels + $this->x_tick_length, $this->ndx_tick_color);
        }

        // Ticks on top of the Plot Area
        if ($this->x_tick_pos == 'plotup' || $this->x_tick_pos == 'both') {
            ImageLine($this->img, $x_pixels, $this->plot_area[1] - $this->x_tick_length, $x_pixels, $this->plot_area[1] + $this->x_tick_cross, $this->ndx_tick_color);
        }

        // Ticks on bottom of Plot Area
        if ($this->x_tick_pos == 'plotdown' || $this->x_tick_pos == 'both') {
            ImageLine($this->img, $x_pixels, $this->plot_area[3] + $this->x_tick_length, $x_pixels, $this->plot_area[3] - $this->x_tick_cross, $this->ndx_tick_color);
        }
        ...
        if ($this->x_tick_label_pos != 'none') {
            ...
            if ($this->x_tick_label_pos == 'xaxis') {
                $this->DrawText('x_label', $this->x_label_angle, $x_pixels, $this->x_axis_y_pixels + $this->x_label_axis_offset, $this->ndx_ticklabel_color, $x_label, 'center', 'top');
            }

            // Label on top of the plot area
            if ($this->x_tick_label_pos == 'plotup' || $this->x_tick_label_pos == 'both') {
                $this->DrawText('x_label', $this->x_label_angle, $x_pixels, $this->plot_area[1] - $this->x_label_top_offset, $this->ndx_ticklabel_color, $x_label, 'center', 'bottom');
            }

            // Label on bottom of the plot area
            if ($this->x_tick_label_pos == 'plotdown' || $this->x_tick_label_pos == 'both') {
                $this->DrawText('x_label', $this->x_label_angle, $x_pixels, $this->plot_area[3] + $this->x_label_bot_offset, $this->ndx_ticklabel_color, $x_label, 'center', 'top');
            }
        }
        return TRUE;
    }
    ...
    protected function DrawYTick($y, $y_pixels) {
        ...
        if ($this->y_tick_pos == 'yaxis') {
            ImageLine($this->img, $this->y_axis_x_pixels - $this->y_tick_length, $y_pixels, $this->y_axis_x_pixels + $this->y_tick_cross, $y_pixels, $this->ndx_tick_color);
        }

        // Ticks to the left of the Plot Area
        if (($this->y_tick_pos == 'plotleft') || ($this->y_tick_pos == 'both')) {
            ImageLine($this->img, $this->plot_area[0] - $this->y_tick_length, $y_pixels, $this->plot_area[0] + $this->y_tick_cross, $y_pixels, $this->ndx_tick_color);
        }

        // Ticks to the right of the Plot Area
        if (($this->y_tick_pos == 'plotright') || ($this->y_tick_pos == 'both')) {
            ImageLine($this->img, $this->plot_area[2] + $this->y_tick_length, $y_pixels, $this->plot_area[2] - $this->y_tick_cross, $y_pixels, $this->ndx_tick_color);
        }
        ...
        if ($this->y_tick_label_pos != 'none') {
            ...
            if ($this->y_tick_label_pos == 'yaxis') {
                $this->DrawText('y_label', $this->y_label_angle, $this->y_axis_x_pixels - $this->y_label_axis_offset, $y_pixels, $this->ndx_ticklabel_color, $y_label, 'right', 'center');
            }

            // Labels to the left of the plot area
            if ($this->y_tick_label_pos == 'plotleft' || $this->y_tick_label_pos == 'both') {
                $this->DrawText('y_label', $this->y_label_angle, $this->plot_area[0] - $this->y_label_left_offset, $y_pixels, $this->ndx_ticklabel_color, $y_label, 'right', 'center');
            }

            // Labels to the right of the plot area
            if ($this->y_tick_label_pos == 'plotright' || $this->y_tick_label_pos == 'both') {
                $this->DrawText('y_label', $this->y_label_angle, $this->plot_area[2] + $this->y_label_right_offset, $y_pixels, $this->ndx_ticklabel_color, $y_label, 'left', 'center');
            }
        }
        return TRUE;
    }
    ...
    protected function DrawPlotBorder($draw_axes = TRUE) {
        ...
        if ($sides == 15) { // Border on all 4 sides
            imagerectangle($this->img, $this->plot_area[0], $this->plot_area[1], $this->plot_area[2], $this->plot_area[3], $this->ndx_grid_color);
        } else {
            if ($sides & 1) // Left
                imageline($this->img, $this->plot_area[0], $this->plot_area[1], $this->plot_area[0], $this->plot_area[3], $this->ndx_grid_color);
            if ($sides & 2) // Top
                imageline($this->img, $this->plot_area[0], $this->plot_area[1], $this->plot_area[2], $this->plot_area[1], $this->ndx_grid_color);
            if ($sides & 4) // Right
                imageline($this->img, $this->plot_area[2], $this->plot_area[1], $this->plot_area[2], $this->plot_area[3], $this->ndx_grid_color);
            if ($sides & 8) // Bottom
                imageline($this->img, $this->plot_area[0], $this->plot_area[3], $this->plot_area[2], $this->plot_area[3], $this->ndx_grid_color);
        }
        return TRUE;
    }
    ...
    protected function DrawDataValueLabel($x_or_y, $row, $column, $x_world, $y_world, $text, $dvl) {
        ...
        if (isset($min_width) || isset($min_height)) {
            ...
            if ((isset($min_width) && ($min_width - $width) < 2) || (isset($min_height) && ($min_height - $height) < 2))
                return FALSE;
        }

        $this->DrawText($font_id, $angle, $this->xtr($x_world) + $x_offset, $this->ytr($y_world) + $y_offset, $this->ndx_dvlabel_color, $formatted_text, $h_align, $v_align);
        return TRUE;
    }
    ...
    protected function DrawXDataLabel($xlab, $xpos, $row, $do_lines = FALSE) {
        ...
        if ($this->x_data_label_pos == 'plotdown' || $this->x_data_label_pos == 'both')
            $this->DrawText('x_label', $this->x_data_label_angle, $xpos, $this->plot_area[3] + $this->x_label_bot_offset, $this->ndx_datalabel_color, $xlab, 'center', 'top');

        // Labels above the plot area
        if ($this->x_data_label_pos == 'plotup' || $this->x_data_label_pos == 'both')
            $this->DrawText('x_label', $this->x_data_label_angle, $xpos, $this->plot_area[1] - $this->x_label_top_offset, $this->ndx_datalabel_color, $xlab, 'center', 'bottom');
    ...
    protected function DrawYDataLabel($ylab, $ypos, $row, $do_lines = FALSE) {
        ...
        if ($this->y_data_label_pos == 'plotleft' || $this->y_data_label_pos == 'both')
            $this->DrawText('y_label', $this->y_data_label_angle, $this->plot_area[0] - $this->y_label_left_offset, $ypos, $this->ndx_datalabel_color, $ylab, 'right', 'center');

        // Labels right of the plot area
        if ($this->y_data_label_pos == 'plotright' || $this->y_data_label_pos == 'both')
            $this->DrawText('y_label', $this->y_data_label_angle, $this->plot_area[2] + $this->y_label_right_offset, $ypos, $this->ndx_datalabel_color, $ylab, 'left', 'center');
    ...
    protected function DrawPieLabel($label_txt, $xc, $yc, $start_angle, $arc_angle, $r) {
        ...
        // Draw the label:
        $this->DrawText('generic', 0, $label_x, $label_y + $yoff, $this->ndx_pielabel_color, $label_txt, $h_align, $v_align);
        return TRUE;
    }
...
/////////////////////////////////////////////
    ...
    function SetLegendColorboxBorders($cbbmode = 'textcolor') {
        $this->legend_colorbox_borders = $this->CheckOption($cbbmode, 'none, textcolor, databordercolor', __FUNCTION__);
    ...
    protected function GetLegendSizeParams() {
        ...
        return compact('width', 'height', 'char_w', 'char_h', 'v_margin', 'colorbox_mode', 'dot_height', 'colorbox_width');
    }
    ...
    protected function GetLegendPosition($width, $height) {
        ...
        switch ($mode) {
            ...
            case 'plot': // SetLegendPosition with mode='plot', relative coordinates over plot area.
                return array((int) ($x_base * $this->plot_area_width - $x * $width) + $this->plot_area[0] + $x_offset,
                    (int) ($y_base * $this->plot_area_height - $y * $height) + $this->plot_area[1] + $y_offset);
    ...
    protected function DrawLegend() {
        ...
        // Draw outer box
        ImageFilledRectangle($this->img, $box_start_x, $box_start_y, $box_end_x, $box_end_y, $this->ndx_legend_bg_color);
        ...
        foreach ($this->legend as $leg) {
            ...
            $this->DrawText('legend', 0, $x_pos, $yc, $this->ndx_legend_text_color, $leg, $this->legend_text_align, 'center');
            if ($colorbox_mode) {
                ...
                if ($this->draw_plot_area_background && $colorbox_mode != 'box') {
                    ImageFilledRectangle($this->img, $dot_left_x, $y1, $dot_right_x, $y2, $this->ndx_plot_bg_color);
                }
                ...
                switch ($colorbox_mode) {
                    ...
                    case 'line':
                        ...
                        $style = $this->SetDashedStyle($this->ndx_data_colors[$color_index], $this->line_styles[$lws_index] == 'dashed');
                    ...
                    default:
                        // Draw color boxes:
                        ImageFilledRectangle($this->img, $dot_left_x, $y1, $dot_right_x, $y2, $this->ndx_data_colors[$color_index]);
...
/////////////////////////////////////////////
    ...
    protected function DrawBar($row, $column, $x1, $y1, $x2, $y2, $data_color, $shade_color, $border_color, $shade_top = TRUE, $shade_side = TRUE) {
    ...
    protected function DrawPieChart() {
        ...
        if ($do_labels && $labels_outside && !$this->pie_full_size) {
            ...
            $avail_width = min(
                    ...
                    $this->label_scale_position, $this->plot_area_width - 4 * $this->safe_margin - 2 * $label_max_width);

            $avail_height = min(
                    ($this->plot_area_height / 2 - $label_max_height - $this->safe_margin - $this->shading) /
                    $this->label_scale_position, $this->plot_area_height - 4 * $this->safe_margin - 2 * ($label_max_height + $this->shading));
...
// Axis data label?
            ...
            for ($idx = 0; $rec < $this->num_recs[$row]; $idx++) {
                if (is_numeric($dv = $this->data[$row][$rec++])) {      // Allow for missing data
                    ...
                    if ($this->datatype_swapped_xy) {
                        ...
                        if ($this->datatype_error_bars) {
                            $this->DrawXErrorBars($dv, $iv, $this->data[$row][$rec], $this->data[$row][$rec + 1], $error_color);
                        }
                    } else {
                        $this->DrawDot($row, $idx, $iv, $dv, $data_color);
                        if ($this->datatype_error_bars) {
                            $this->DrawYErrorBars($iv, $dv, $this->data[$row][$rec], $this->data[$row][$rec + 1], $error_color);
                        }
                    }
            ...
            for ($idx = 0; $rec < $this->num_recs[$row]; $rec++, $idx++) {
                if (is_numeric($dv = $this->data[$row][$rec])) {          // Allow for missing data
                    ...
                    if ($this->datatype_swapped_xy) {
                        ...
                        ImageLine($this->img, $this->y_axis_x_pixels, $y_now_pixels, $this->xtr($dv), $y_now_pixels, $data_color);
                    } else {
                        // Draw a line from user defined x axis position up (or down) to ytr($dv)
                        ImageLine($this->img, $x_now_pixels, $this->x_axis_y_pixels, $x_now_pixels, $this->ytr($dv), $data_color);
                    }
                }
            }
        }
    ...
    protected function DrawArea($do_stacked = FALSE) {
        ...
        if ($n < 2)
            return TRUE;    // Require at least 2 rows, for imagefilledpolygon().
...
// Convert independent variable to device coordinates
            ...
            for ($idx = 0; $rec < $this->num_recs[$row]; $idx++) {
                ...
                if (is_numeric($dv = $this->data[$row][$rec++]) && $line_style != 'none') {
                    ...
                    if ($start_lines[$idx]) {
                        ...
                        ImageLine($this->img, $x_now_pixels, $y_now_pixels, $lastx[$idx], $lasty[$idx], $style);
                    }
    ...
    protected function DrawOHLC($draw_candles, $always_fill = FALSE) {
        ...
        $dw = max($this->ohlc_min_width, min($this->ohlc_max_width, (int) ($this->ohlc_frac_width * $this->plot_area_width / $this->num_data_rows)));
        ...
        for ($row = 0; $row < $this->num_data_rows; $row++) {
            ...
            if (!is_numeric($yo = $this->data[$row][$record++]) || !is_numeric($yh = $this->data[$row][$record++]) || !is_numeric($yl = $this->data[$row][$record++]) || !is_numeric($yc = $this->data[$row][$record++])) {
                continue;
            }
    ...
    protected function DrawBoxes() {
        ...
        // This is scaled based on the plot density, but within tight limits.
        $width1 = max($this->boxes_min_width, min($this->boxes_max_width, (int) ($this->boxes_frac_width * $this->plot_area_width / $this->num_data_rows)));
