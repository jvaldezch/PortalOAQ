...
class OAQ_Vucem {
    ...
    protected function rfcsConsulta($rfcs, $cadena = null) {
        $xmlRfcs = '';
        if(!isset($cadena)) {            
            if (is_array($rfcs)) {
                foreach ($rfcs as $r) {
    ...
    protected function rfcsConsultaDig($rfcs, $cadena = null) {
        $xmlRfcs = '';
        if(!isset($cadena)) {            
            if (is_array($rfcs)) {
                foreach ($rfcs as $r) {
                    $xmlRfcs .= "<dig:rfcConsulta>" . $r . "</dig:rfcConsulta>";
                }
            } else {
                $xmlRfcs .= "<dig:rfcConsulta>" . $rfcs . "</dig:rfcConsulta>";
            }            
        } else {
            foreach ($rfcs as $r) {
                $xmlRfcs .= $r . "|";
    ...
    public function crearComprobante($factura, $tipoFigura, $tipoOp, $rfcs, $emails, $cert, $pkeyid, $patente = null, $adenda = null) {
        try {
            $xml = '';
            $xmlRfcs = $this->rfcsConsulta($rfcs);
            $xmlEmails = $this->emails($emails);

            $emisor = $this->crearEmisor($factura);
            $dest = $this->crearDestinatario($factura);
            $merc = $this->crearProductos($factura["Productos"], $factura["ValDls"]);
            $tmpXml = "<oxml:solicitarRecibirCoveServicio>";
            $arrXml = $this->preXml($tipoOp, $patente, $factura["FechaFactura"], $xmlRfcs, $tipoFigura, $xmlEmails, $factura["NumFactura"], $factura["CertificadoOrigen"], $factura["NumExportador"], $factura["Subdivision"], $emisor, $dest, $merc, null, null, null, null, (isset($factura["Manual"])) ? $factura["Manual"] : null, $adenda, (isset($factura["Observaciones"])) ? $factura["Observaciones"] : null);

            $tmpXml .= $arrXml["xml"];
            $tmpXml .= "</oxml:solicitarRecibirCoveServicio>";
            $cadena = $this->cadenaOriginal($tmpXml);

            $xml = $this->preXml($tipoOp, $patente, $factura["FechaFactura"], $xmlRfcs, $tipoFigura, $xmlEmails, $factura["NumFactura"], $factura["CertificadoOrigen"], $factura["NumExportador"], $factura["Subdivision"], $emisor, $dest, $merc, true, $cert, $pkeyid, $cadena, (isset($factura["Manual"])) ? $factura["Manual"] : null, $adenda, (isset($factura["Observaciones"])) ? $factura["Observaciones"] : null);
    ...
    protected function fixCadena($cadena) {
        /* $cad = str_replace(array('&amp;'), '&', $cadena);        
          return $cad; */
        /* $cadena = strtr($cadena,array('&amp;'=>'&'));
          $entities = htmlentities($cadena,ENT_QUOTES, "UTF-8");
          return html_entity_decode($entities); */
        //$decoded = html_entity_decode($cadena,ENT_QUOTES,"UTf-8");
        //$special = array('/&/','/Ñ/');
        /* $special = array('/&/');
          //$replace = array('&#38;','&#209;');
          //$replace = array('&amp;','&Ntilde;');
          $replace = array('&amp;');
          return preg_replace( $special, $replace, $cadena ); */
        //Zend_Debug::dump( utf8_decode($cadena) );
        //die();
        //return utf8_encode($cadena);
        //return html_entity_decode($cadena,ENT_QUOTES,"UTf-8");
        //return $cadena;
        //Zend_Debug::dump( strtr(utf8_decode($cadena),array('&amp;'=>'&')) );
        //return strtr($cadena,array('&amp;'=>'&'));
        return strtr(utf8_decode($cadena), array('&amp;' => '&'));
    }

    protected function preXml($tipoOp, $patente, $fechaFact, $xmlRfcs, $tipoFigura, $xmlEmails, $numFact, $certOri, $numExp, $subDiv, $emisor, $dest, $merc, $firma = null, $cert = null, $pkeyid = null, $cadena = null, $manual = null, $adenda = null, $obs = null) {
        try {
            $xml = '';
            $firmaXml = '';
            $firmaElect = '';
            ...
            } elseif ($tipoOp == 'TOCE.EXP' && !isset($manual)) {
            ...
            $xml = "<oxml:comprobantes>{$adenda}
                            ...
                            <oxml:patenteAduanal>" . $patente . "</oxml:patenteAduanal>
                            <oxml:fechaExpedicion>" . date("Y-m-d", strtotime($fechaFact)) . "</oxml:fechaExpedicion>{$obs}
                            " . $xmlRfcs . "
                            <oxml:tipoFigura>" . $tipoFigura . "</oxml:tipoFigura>
                            " . $xmlEmails . "" . $firmaXml . "
                            <oxml:numeroFacturaOriginal>" . $this->vuChar($numFact, true) . "</oxml:numeroFacturaOriginal>
                            <oxml:factura>
                                <oxml:certificadoOrigen>" . $certOri . "</oxml:certificadoOrigen>" . $exportador . "
    ...
    protected function preXmlRelacion($numRelacion, $facturas, $tipoOp, $xmlRfcs, $xmlEmails, $tipoFigura, $emisor, $dest, $firma = null, $cadena = null, $cert = null, $pkeyid = null) {
        try {
            ...
            $tmpXml .= "\n\t\t    <oxml:tipoOperacion>" . $tipoOp . "</oxml:tipoOperacion>
                    ...
                    <oxml:tipoFigura>" . $tipoFigura . "</oxml:tipoFigura>
                    " . $xmlEmails . "" . $firmaXml . "
                    <oxml:numeroRelacionFacturas>{$numRelacion}</oxml:numeroRelacionFacturas>
...
//            }
    ...
    protected function cadenaOriginal($xml) {
        try {
            ...
            $cadena = $xslt->transformToXml(new SimpleXMLElement($cleanXml));
            ...
            if (isset($cleanXml))
                Zend_Debug::dump($cleanXml);
            echo "<b>Exception on " . __METHOD__ . "</b>" . $e->getMessage();
            die();
        }
    }

    public function crearProductos($productos, $valDls = null) {
        try {
            $misc = new OAQ_Misc();
            $xml = '';
            ...
            foreach ($productos as $prod) {
                ...
                if (isset($prod["DESC"])) {
                    $xml .= "\t\t\t\t<oxml:descripcionGenerica>" . $misc->trimUc($prod["DESC"]) . "</oxml:descripcionGenerica>\n";
                } elseif (isset($prod["DESC1"])) {
                    $xml .= "\t\t\t\t<oxml:descripcionGenerica>" . $misc->trimUc($prod["DESC1"]) . "</oxml:descripcionGenerica>\n";
                } elseif (isset($prod["DESC_COVE"])) {
                    $xml .= "\t\t\t\t<oxml:descripcionGenerica>" . $prod["DESC_COVE"] . "</oxml:descripcionGenerica>\n";
                }
                if (isset($prod["PARTE"]) && $misc->trimUc($prod["PARTE"]) != '') {
                    $xml .= isset($prod["PARTE"]) ? "\t\t\t\t<oxml:numparte>" . strtoupper($prod["PARTE"]) . "</oxml:numparte>\n" : '';
                }
                if (isset($prod["ORDEN"]) && $misc->trimUc($prod["ORDEN"]) != '') {
                    $xml .= isset($prod["ORDEN"]) ? "\t\t\t\t<oxml:secuencial>" . strtoupper($prod["ORDEN"]) . "</oxml:secuencial>\n" : '';
                }
                $xml .= isset($prod["UMC_OMA"]) ? "\t\t\t\t<oxml:claveUnidadMedida>" . strtoupper($prod["UMC_OMA"]) . "</oxml:claveUnidadMedida>\n" : '';
...
//                if (isset($prod["OBS"]) && $prod["OBS"] != "" && (preg_match('/Marca:/i', $prod["OBS"]) || preg_match('/Modelo:/i', $prod["OBS"]) || preg_match('/Numero de Serie:/i', $prod["OBS"]) || preg_match('/Sub Modelo:/i', $prod["OBS"]) )) {
//                    $xml .= $this->observaciones($prod["OBS"]);
//                }
    ...
    protected function vuChar($value, $special = null) {
        /* if($special==true && preg_match('/&/', $value)) {
          return str_replace('&', '&amp;', $value);
          }
          return strtoupper(utf8_decode($value)); */
        return $value;
    }
    ...
    protected function crearEmisor($factura, $tipoOperacion = null) {
        ...
        //$emisor = "\n\t\t\t\t<oxml:tipoIdentificador>".$this->tipoIdentificador($factura["CteRfc"], $factura["CtePais"])."</oxml:tipoIdentificador>\n";
        //$emisor .= "\t\t\t\t<oxml:identificacion>".$factura["CteRfc"]."</oxml:identificacion>\n";
//        $emisor .= "\t\t\t\t<oxml:nombre>" . utf8_decode($factura["CteNombre"]) . "</oxml:nombre>\n";
//        $emisor .= "\t\t\t\t<oxml:domicilio>\n";
//        if (isset($factura["CteCalle"]) && $factura["CteCalle"] != "") {
//            $emisor .= "\t\t\t\t\t<oxml:calle>" . utf8_decode($factura["CteCalle"]) . "</oxml:calle>\n";
//        }
//        if (isset($factura["CteNumExt"]) && $factura["CteNumExt"] != "") {
//            $emisor .= "\t\t\t\t\t<oxml:numeroExterior>" . utf8_decode($factura["CteNumExt"]) . "</oxml:numeroExterior>\n";
//        }
//        if (isset($factura["CteNumInt"]) && $factura["CteNumInt"] != "") {
//            $emisor .= "\t\t\t\t\t<oxml:numeroInterior>" . utf8_decode($factura["CteNumInt"]) . "</oxml:numeroInterior>\n";
//        }
//        if (isset($factura["CteColonia"]) && $misc->trimUcUtf8($factura["CteColonia"]) != "") {
//            $emisor .= "\t\t\t\t\t<oxml:colonia>" . $misc->trimUcUtf8($factura["CteColonia"]) . "</oxml:colonia>\n";
//        }
//        if (isset($factura["CteLocalidad"]) && $factura["CteLocalidad"] != "") {
//            $emisor .= "\t\t\t\t\t<oxml:localidad>" . utf8_decode($factura["CteLocalidad"]) . "</oxml:localidad>\n";
//        }
//        if (isset($factura["CteMun"]) && $factura["CteMun"] != "") {
//            $emisor .= "\t\t\t\t\t<oxml:municipio>" . utf8_decode($factura["CteMun"]) . "</oxml:municipio>\n";
//        }
//        if (isset($factura["CteEdo"]) && $factura["CteEdo"] != "") {
//            $emisor .= "\t\t\t\t\t<oxml:entidadFederativa>" . utf8_decode($factura["CteEdo"]) . "</oxml:entidadFederativa>\n";
//        }
//        if (isset($factura["CtePais"]) && $factura["CtePais"] != "") {
//            $emisor .= "\t\t\t\t\t<oxml:pais>" . utf8_decode($factura["CtePais"]) . "</oxml:pais>\n";
//        }
//        if (isset($factura["CteCP"]) && $factura["CteCP"] != "") {
//            $emisor .= "\t\t\t\t\t<oxml:codigoPostal>" . utf8_decode($factura["CteCP"]) . "</oxml:codigoPostal>\n";
//        }
//        $emisor .= "\t\t\t\t</oxml:domicilio>\n";
//        return $this->htmlspanishchars(utf8_encode($emisor));
        ...
        $data .= "\t\t\t\t</oxml:domicilio>\n";
        return $this->htmlspanishchars(utf8_encode($data));
    }
    ...
    protected function crearDestinatario($factura, $tipoOperacion = null) {
        ...
        //$dest = "\n\t\t\t\t<oxml:tipoIdentificador>".$this->tipoIdentificador($factura["ProTaxID"], $factura["ProPais"])."</oxml:tipoIdentificador>\n";
        //$dest .= "\t\t\t\t<oxml:identificacion>".$factura["ProTaxID"]."</oxml:identificacion>\n";
//        $dest .= "\t\t\t\t<oxml:nombre>" . utf8_decode($factura["ProNombre"]) . "</oxml:nombre>\n";
//        $dest .= "\t\t\t\t<oxml:domicilio>\n";
//        if (isset($factura["ProCalle"]) && $factura["ProCalle"] != "") {
//            $dest .= "\t\t\t\t\t<oxml:calle>" . utf8_decode($factura["ProCalle"]) . "</oxml:calle>\n";
//        }
//        if (isset($factura["ProNumExt"]) && $factura["ProNumExt"] != "") {
//            $dest .= "\t\t\t\t\t<oxml:numeroExterior>" . utf8_decode($factura["ProNumExt"]) . "</oxml:numeroExterior>\n";
//        }
//        if (isset($factura["ProNumInt"]) && $factura["ProNumInt"] != "") {
//            $dest .= "\t\t\t\t\t<oxml:numeroInterior>" . utf8_decode($factura["ProNumInt"]) . "</oxml:numeroInterior>\n";
//        }
//        if (isset($factura["ProColonia"]) && $factura["ProColonia"] != "") {
//            $dest .= "\t\t\t\t\t<oxml:colonia>" . utf8_decode($factura["ProColonia"]) . "</oxml:colonia>\n";
//        }
//        if (isset($factura["ProLocalidad"]) && $factura["ProLocalidad"] != "") {
//            $dest .= "\t\t\t\t\t<oxml:localidad>" . utf8_decode($factura["ProLocalidad"]) . "</oxml:localidad>\n";
//        }
//        if ($factura["ProPais"] == "MEX") {
//            if (isset($factura["ProMun"]) && $factura["ProMun"] != "") {
//                $dest .= "\t\t\t\t\t<oxml:municipio>" . utf8_decode($factura["ProMun"]) . "</oxml:municipio>\n";
//            }
//            if (isset($factura["ProEdo"]) && $factura["ProEdo"] != "") {
//                $dest .= "\t\t\t\t\t<oxml:entidadFederativa>" . utf8_decode($factura["ProEdo"]) . "</oxml:entidadFederativa>\n";
//            }
//        } else {
//            if (isset($factura["ProMun"]) && $factura["ProMun"] != "") {
//                $dest .= "\t\t\t\t\t<oxml:municipio>" . utf8_decode($factura["ProMun"]) . "</oxml:municipio>\n";
...
//            }
//        }
//        if (isset($factura["ProPais"]) && $factura["ProPais"] != "") {
//            $dest .= "\t\t\t\t\t<oxml:pais>" . utf8_decode($factura["ProPais"]) . "</oxml:pais>\n";
//        }
//
//        if (isset($factura["ProCP"]) && $factura["ProCP"] != "") {
//            $dest .= "\t\t\t\t\t<oxml:codigoPostal>" . utf8_decode($factura["ProCP"]) . "</oxml:codigoPostal>\n";
//        }
//        $dest .= "\t\t\t\t</oxml:domicilio>\n";
        //return $this->htmlspanishchars(utf8_encode($dest));        
    ...
    protected function datosEmisorDestinatario($array, $arrayField, $xmlField, $dom = null) {
        ...
        if (isset($array[$arrayField]) && $misc->trimUcUtf8($array[$arrayField]) != "" && !$dom) {
            return "\t\t\t\t<oxml:{$xmlField}>" . utf8_decode($array[$arrayField]) . "</oxml:{$xmlField}>\n";
     ...
     * @return String
     */
    public function vucemServicio($xml, $url) {
        try {
            $headers = array(
                "Content-type: text/xml; charset=UTF-8",
                "Accept: text/xml",
...
//            curl_setopt($soap, CURLOPT_SSL_VERIFYHOST, false);
     ...
     * @return String
     */
    public function solicitarRespuestaCove($xml, $url = null) {
        try {
            $headers = array(
                "Content-type: text/xml; charset=UTF-8",
                "Accept: text/xml",
...
//    public function vucemObtenerPedimentos($patente,$aduana,$fecha,$rfc,$passWs)
//    {
...
//        
//        $dbVucem = new Vucem_Model_VucemPeticionesMapper();
//        $id = $dbVucem->nuevaPeticion($xml);
//        
//        $soap = curl_init();
//        curl_setopt($soap, CURLOPT_URL, $url);
//        curl_setopt($soap, CURLOPT_RETURNTRANSFER, true);
...
//
//        $result = curl_exec($soap);
//        $xmlArray = $this->vucemXmlToArray($result);
//        $error = 0;
//        $mensaje = null;
//        if($xmlArray["Body"]["consultarPedimentosRespuesta"]["tieneError"] == 'true') $error = 1;
//        if($error == 1) {
//            $mensaje = $xmlArray["Body"]["consultarPedimentosRespuesta"]["error"]["mensaje"];
//        }
//        
//        if($result) $dbVucem->actualizarPeticion($id, $result, $error, $mensaje);
//        unset($result);
//        curl_close($soap);
//        
//        $xmlArray['PeticionId'] = $id;
//        return $xmlArray;
//    }
//    public function vucemPedimentoCompleto($aduana,$patente,$pedimento,$rfc,$passWs)
//    {
...
//        $error = 0;
//        $mensaje = null;
//        if($xmlArray["Body"]["consultarPedimentoCompletoRespuesta"]["tieneError"] == 'true') $error = 1;
//        if($error == 1) {
//            $mensaje = $xmlArray["Body"]["consultarPedimentoCompletoRespuesta"]["error"]["mensaje"];
//        }        
//        if($result) {
//            $dbVucem->actualizarPeticion($id, $result, $error, $mensaje);
//            $ped = new Vucem_Model_VucemPedimentosMapper();
//            $ped->actualizarPedimento($result, $patente, $aduana, $pedimento,$id);
//            unset($ped);
//        }
//        unset($result);
//        curl_close($soap);        
...
//    }
    ...
    public function vucemXmlToArray($xml) {
        try {
            $clean = str_replace(array('ns2:', 'ns3:', 'ns9:', 'ns8:', 'S:', 'wsu:', 'wsse:', 'xmlns:', 'ns3:', 'wsu:', 'soapenv:', 'env:', 'oxml:', '<![CDATA[', ']]>'), '', $xml);
...
//            curl_setopt($soap, CURLOPT_SSL_VERIFYHOST, false);
    ...
    public function prepararEnvioRelacion($comprobantes, $username, $password) {
        try {
            ...
            return array(
                'xml' => $xml,
                'cert' => $comprobantes["cert"],
                'cadena' => $comprobantes["cadena"],
                'firma' => $comprobantes["firma"],
            );
        } catch (Exception $e) {
            echo "<b>Exception on " . __METHOD__ . "</b>" . $e->getMessage();
            die();
   ...
   <soapenv:Header>
    ...
    public function envioEdocument($username, $password, $correoElectronico, $idTipoDocumento, $nombreDocumento, $rfcConsulta, $archivo, $certificado, $cadenaOriginal, $firma) {
        if(!is_array($rfcConsulta)) {
            ...
            $rfcs = $this->rfcsConsultaDig($rfcConsulta);
            $xml = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:dig=\"http://www.ventanillaunica.gob.mx/aga/digitalizar/ws/oxml/DigitalizarDocumento\" xmlns:res=\"http://www.ventanillaunica.gob.mx/common/ws/oxml/respuesta\">
                <soapenv:Header>
                     <wsse:Security soapenv:mustUnderstand=\"1\" xmlns:wsse=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd\">
                             <wsse:UsernameToken>
   ...
   <soapenv:Body>
    ...
    public function cadenaEdocument($rfcFirmante, $correoElectronico, $idTipoDocumento, $nombreDocumento, $rfcConsulta, $hash) {
        if(is_array($rfcConsulta)) {
            $rfcs = $this->rfcsConsultaDig($rfcConsulta,true);
        } else {
            $rfcs = $rfcConsulta . '|';
        }        
        $cadena = "|{$rfcFirmante}|{$correoElectronico}|{$idTipoDocumento}|{$nombreDocumento}|{$rfcs}{$hash}|";
        return $cadena;
    }
    ...
    public function printCove($id,$download,$view,$save) {
        ...
        $array = $vucem->vucemXmlToArray($xml["xml"]);
        ...
        $html = '<h3 style="text-align:center; line-height: 12px; margin:0; padding: 0;">DATOS DEL COMPROBANTE</h3>'
                . "<p style=\"text-align:center; line-height: 12px; margin:0; padding: 0;\"><strong>REFERENCIA:</strong> {$xml["referencia"]}, <strong>PEDIMENTO:</strong> {$xml["pedimento"]}</p>"
                . '<table>'
                . '<tr>'
        ...
        $html .= '<table>'
                ...
                . '<tr>'
                . '<td style="' . $tdn . '">' . ( (isset($data["factura"]["subdivision"]) && $data["factura"]["subdivision"] != '0') ? 'CON SUBDIVISIÓN' : 'SIN SUBDIVISIÓN' ) . '</td>
                    <td style="' . $tdn . '">' . ( (isset($data["factura"]["certificadoOrigen"]) && $data["factura"]["certificadoOrigen"] != '0') ? "SI FUNGE COMO CERTIFICADO DE ORIGEN" : 'NO FUNGE COMO CERTIFICADO DE ORIGEN' ) . '</td>
        ...
        $html .= '<br /><br /><table>'
                ...
                <tr>
                    <td style="' . $tdn . '">' . ( (isset($data["emisor"]["nombre"]) && $data["emisor"]["nombre"] != '') ? $data["emisor"]["nombre"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '">&nbsp;</td>
                ...
                <tr>
                    <td style="' . $tdn . '">' . ( (isset($data["emisor"]["domicilio"]["calle"]) && $data["emisor"]["domicilio"]["calle"] != '') ? $data["emisor"]["domicilio"]["calle"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '">' . ( (isset($data["emisor"]["domicilio"]["numeroExterior"]) && $data["emisor"]["domicilio"]["numeroExterior"] != '') ? $data["emisor"]["domicilio"]["numeroExterior"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '">' . ( (isset($data["emisor"]["domicilio"]["numeroInterior"]) && $data["emisor"]["domicilio"]["numeroInterior"] != '') ? $data["emisor"]["domicilio"]["numeroInterior"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '">' . ( (isset($data["emisor"]["domicilio"]["codigoPostal"]) && $data["emisor"]["domicilio"]["codigoPostal"] != '') ? $data["emisor"]["domicilio"]["codigoPostal"] : '&nbsp;' ) . '</td>
                </tr>
                <tr>
                    <th style="' . $thtitle . '">COLONIA</th>
                    <th style="' . $thtitle . '" colspan="3">LOCALIDAD (CIUDAD)</th>
                </tr>
                <tr>
                    <td style="' . $tdn . '">' . ( (isset($data["emisor"]["domicilio"]["colonia"]) && $data["emisor"]["domicilio"]["colonia"] != '') ? $data["emisor"]["domicilio"]["colonia"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '" colspan="3">' . ( (isset($data["emisor"]["domicilio"]["localidad"]) && $data["emisor"]["domicilio"]["localidad"] != '') ? $data["emisor"]["domicilio"]["localidad"] : '&nbsp;' ) . '</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td style="' . $tdn . '">' . ( (isset($data["emisor"]["domicilio"]["municipio"]) && $data["emisor"]["domicilio"]["municipio"] != '') ? $data["emisor"]["domicilio"]["municipio"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '" colspan="3">' . ( (isset($data["emisor"]["domicilio"]["entidadFederativa"]) && $data["emisor"]["domicilio"]["entidadFederativa"] != '') ? $data["emisor"]["domicilio"]["entidadFederativa"] : '&nbsp;' ) . '</td>
                </tr>
                <tr>
                    <th style="' . $thtitle . '" colspan="4">PAÍS</th>
                </tr>
                <tr>
                    <td style="' . $tdn . '" colspan="4">' . ( (isset($data["emisor"]["domicilio"]["pais"]) && $data["emisor"]["domicilio"]["pais"] != '') ? $data["emisor"]["domicilio"]["pais"] : '&nbsp;' ) . '</td>
                </tr>'
                . '</table>';
        ...
        $html .= '<br /><br /><table>'
                ...
                <tr>
                    <td style="' . $tdn . '">' . ( (isset($data["destinatario"]["nombre"]) && $data["destinatario"]["nombre"] != '') ? $data["destinatario"]["nombre"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '">&nbsp;</td>
                    <td style="' . $tdn . '" colspan="2">&nbsp;</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td style="' . $tdn . '">' . ( (isset($data["destinatario"]["domicilio"]["calle"]) && $data["destinatario"]["domicilio"]["calle"] != '') ? $data["destinatario"]["domicilio"]["calle"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '">' . ( (isset($data["destinatario"]["domicilio"]["numeroExterior"]) && $data["destinatario"]["domicilio"]["numeroExterior"] != '') ? $data["destinatario"]["domicilio"]["numeroExterior"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '">' . ( (isset($data["destinatario"]["domicilio"]["numeroInterior"]) && $data["destinatario"]["domicilio"]["numeroInterior"] != '') ? $data["destinatario"]["domicilio"]["numeroInterior"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '">' . ( (isset($data["destinatario"]["domicilio"]["codigoPostal"]) && $data["destinatario"]["domicilio"]["codigoPostal"] != '') ? $data["destinatario"]["domicilio"]["codigoPostal"] : '&nbsp;' ) . '</td>
                </tr>
                <tr>
                    <th style="' . $thtitle . '">COLONIA</th>
                    <th style="' . $thtitle . '" colspan="3">LOCALIDAD (CIUDAD)</th>
                ...
                <tr>
                    <td style="' . $tdn . '">' . ( (isset($data["destinatario"]["domicilio"]["colonia"]) && $data["destinatario"]["domicilio"]["colonia"] != '') ? $data["destinatario"]["domicilio"]["colonia"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '" colspan="3">' . ( (isset($data["destinatario"]["domicilio"]["localidad"]) && $data["destinatario"]["domicilio"]["localidad"] != '') ? $data["destinatario"]["domicilio"]["localidad"] : '&nbsp;' ) . '</td>
                </tr>
                <tr>
                    <th style="' . $thtitle . '">MUNICIPIO</th>
                    <th style="' . $thtitle . '" colspan="3">ENTIDAD FEDERATIVA (ESTADO)</th>
                ...
                <tr>        
                    <td style="' . $tdn . '">' . ( (isset($data["destinatario"]["domicilio"]["municipio"]) && $data["destinatario"]["domicilio"]["municipio"] != '') ? $data["destinatario"]["domicilio"]["municipio"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '" colspan="3">' . ( (isset($data["destinatario"]["domicilio"]["entidadFederativa"]) && $data["destinatario"]["domicilio"]["entidadFederativa"] != '') ? $data["destinatario"]["domicilio"]["entidadFederativa"] : '&nbsp;' ) . '</td>
                </tr>
                <tr>
                    <th style="' . $thtitle . '" colspan="4">PAÍS</th>
                </tr>
                <tr>
                    <td style="' . $tdn . '" colspan="4">' . ( (isset($data["destinatario"]["domicilio"]["pais"]) && $data["destinatario"]["domicilio"]["pais"] != '') ? $data["destinatario"]["domicilio"]["pais"] : '&nbsp;' ) . '</td>
                </tr>'
                . '</table>';
        ...
        if (isset($data["mercancias"]["descripcionGenerica"])) {
            $html = '<table>'
                    . '<tr nobr="true">
                <td style="' . $tdn . '">' . ((isset($data["mercancias"]["descripcionGenerica"]) && $data["mercancias"]["descripcionGenerica"] != '') ? $data["mercancias"]["descripcionGenerica"] : '&nbsp;' ) . '</td>
                <td style="' . $tdn . '">' . ((isset($data["mercancias"]["numparte"]) && $data["mercancias"]["numparte"] != '') ? $data["mercancias"]["numparte"] : '&nbsp;' ) . '</td>
                <td style="' . $tdn . '">' . ((isset($data["mercancias"]["tipoMoneda"]) && $data["mercancias"]["tipoMoneda"] != '') ? $data["mercancias"]["tipoMoneda"] : '&nbsp;' ) . '</td>
                ...
                <td style="' . $tdn . '">' . ((isset($data["mercancias"]["claveUnidadMedida"]) && $data["mercancias"]["claveUnidadMedida"] != '') ? $data["mercancias"]["claveUnidadMedida"] : '&nbsp;' ) . '</td>
            ...
            foreach ($data["mercancias"] as $merc) {
                $html = '<table>
                    <tr nobr="true">
                    <td style="' . $tdn . '">' . ( (isset($merc["descripcionGenerica"]) && $merc["descripcionGenerica"] != '') ? $merc["descripcionGenerica"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '">' . ( (isset($merc["numparte"]) && $merc["numparte"] != '') ? $merc["numparte"] : '&nbsp;' ) . '</td>
                    <td style="' . $tdn . '">' . ( (isset($merc["tipoMoneda"]) && $merc["tipoMoneda"] != '') ? $merc["tipoMoneda"] : '&nbsp;' ) . '</td>
                    ...
                    <td style="' . $tdn . '">' . ( (isset($merc["claveUnidadMedida"]) && $merc["claveUnidadMedida"] != '') ? $merc["claveUnidadMedida"] : '&nbsp;' ) . '</td>
        ...
        $html .= '<table>'
                ...
                <tr nobr="true">
                    <td style="' . $tdn . '" class="signature">' . ((isset($data["firmaElectronica"]["cadenaOriginal"]) && $data["firmaElectronica"]["cadenaOriginal"] != '') ? wordwrap($data["firmaElectronica"]["cadenaOriginal"], 100, "<br />", true) : '&nbsp;' ) . '</td>
                </tr>
                <tr>
        ...
        if (isset($view)) {
            // https://192.168.0.246/automatizacion/vucem/print-cove?id=1974&view=true
            $pdf->Output('/tmp' . DIRECTORY_SEPARATOR . $xml["cove"] . '.pdf', 'I');
            unlink('/tmp' . DIRECTORY_SEPARATOR . $xml["cove"] . '.pdf');
        } elseif (isset($download)) {
            // https://192.168.0.246/automatizacion/vucem/print-cove?id=1974&download=true
            $pdf->Output('/tmp' . DIRECTORY_SEPARATOR . $xml["cove"] . '.pdf', 'FD');
            unlink('/tmp' . DIRECTORY_SEPARATOR . $xml["cove"] . '.pdf');
        } elseif (isset($save)) {
            // https://192.168.0.246/automatizacion/vucem/print-cove?id=1974&save=true
            ...
            if (file_exists($filenamePdf)) {
                if (!($arch->checkIfFileExists($xml["referencia"], $xml["patente"], $xml["aduana"], basename($filenamePdf)))) {
                    $arch->addNewFile(22, null, $xml["referencia"], $xml["patente"], $xml["aduana"], basename($filenamePdf), $filenamePdf, (isset($session)) ? $session->username : null);
                }
            }
            if (!file_exists($filenameXml)) {
                ...
                if (file_exists($filenameXml)) {
                    if (!($arch->checkIfFileExists($xml["referencia"], $xml["patente"], $xml["aduana"], basename($filenameXml)))) {
                        $arch->addNewFile(21, null, $xml["referencia"], $xml["patente"], $xml["aduana"], basename($filenameXml), $filenameXml, (isset($session)) ? $session->username : null);
                    }
                }
            }            
        }
    ...
    public function printEdoc($uuid,$solicitud,$download,$view,$save) {
...
EOD;
        ...
        if (isset($view)) {
            ...
            $arch = new Archivo_Model_RepositorioMapper();
            $acuseEdoc = $folder . DIRECTORY_SEPARATOR . 'EDOC_' . $data["edoc"] . '.pdf';
            ...
            if (file_exists($acuseEdoc)) {
                if (!($arch->checkIfFileExists($data["referencia"], $data["patente"], $data["aduana"], basename($acuseEdoc)))) {
                    $arch->addNewFile(27, null, $data["referencia"], $data["patente"], $data["aduana"], basename($acuseEdoc), $acuseEdoc, (isset($session)) ? $session->username : null, $data["edoc"]);
                    ...
                    if (!file_exists($digitalizado)) {
                        ...
                        $arch->addNewFile($file["tipoDoc"], null, $data["referencia"], $data["patente"], $data["aduana"], $data["nomArchivo"], $digitalizado, (isset($session)) ? $session->username : null, $data["edoc"]);
                    }
                }
            }
        }
