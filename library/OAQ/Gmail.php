...
class OAQ_Gmail {
    ...
    function __construct($user, $pass) {
        $this->_user = $user;
        $this->_pass = $pass;
        $this->_mbox = imap_open('{imap.gmail.com:993/imap/ssl}INBOX', $this->_user, $this->_pass) or die("Unable to connect Gmail: " . imap_last_error());
        imap_sort($this->_mbox, SORTDATE, 1);
        imap_sort($this->_mbox, SORTARRIVAL, 1);
    }
    
    public function search($search) {
        return imap_search($this->_mbox, $search);
    }
    ...
    public function getFolders() {
        return imap_list($this->_mbox, '{imap.gmail.com:993/imap/ssl}', "*");
    }
