...
class OAQ_Servicios {
    ...
    protected $pedimentoEstadoEndpoint = "https://www.ventanillaunica.gob.mx/ventanilla-ws-pedimentos/ConsultarEstadoPedimentosService";
    ...
    protected $consultaEdocEndpoint = "https://www.ventanillaunica.gob.mx/ventanilla/DigitalizarDocumentoService";
    protected $descargaEdocEndpoint = "https://www.ventanillaunica.gob.mx/Ventanilla-HA/ServicioEdocument/ServicioEdocument.svc";
    protected $aduana;
    protected $patente;
    protected $pedimento;
    protected $xml;
    ...
    public function descargaEdocument() {
        try {
            $headers = array(
                "Content-type: text/xml; charset=UTF-8",
                "Accept: text/xml",
                ...
                "Content-length: " . strlen($this->xml) . "");
            $soap = curl_init();
            curl_setopt($soap, CURLOPT_URL, $this->descargaEdocEndpoint);
            curl_setopt($soap, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($soap, CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($soap, CURLOPT_SSL_VERIFYHOST, false);
            curl_setopt($soap, CURLOPT_CONNECTTIMEOUT ,0); 
    ...
    public function consultaEstadoPedimento() {
        try {
            $this->_consumirServicio($this->pedimentoEstadoEndpoint);
        } catch (Exception $e) {
            throw new Exception("Exception on " . __METHOD__ . ": " . $e->getMessage());
        }
    }
    ...
    public function consultaEdocument() {
        try {
            $this->_consumirServicio($this->consultaEdocEndpoint);
        } catch (Exception $e) {
            throw new Exception("Exception on " . __METHOD__ . ": " . $e->getMessage());
        }
    }
