...
class OAQ_Archivos_PlantillaFacturas
{
    ...
    public function set_idTrafico($_idTrafico)
    {
        $this->_idTrafico = $_idTrafico;
    }

    public function set_replace($_replace)
    {
    ...
    protected function _insertProducto($idFactura, $producto)
    {
        try {
            $arr = array(
                ...
                "precioUnitario" => (float) $producto["PRECIO_UNI"],
                "valorComercial" => (float) $producto["VAL_COM"],
                ...
                "umt" => $producto["UMT"],
                "cantidadOma" => (float) $producto["CAN_OMA"],
    ...
    protected function _insertFacturaDetalle($idFactura, $idPro, $factura)
    {
        try {
            ...
            $arr = array(
                ...
                "factorMonExt" => (float) $factura["VAL_EQUI"],
    ...
    protected function _insertFactura($factura)
    {
        try {
            $arr = array(
                ...
                "valorDolares" => (float) trim($factura["VAL_DLS"]),
                "valorMonExt" => (float) trim($factura["VAL_EXT"]),
                "factorMonExt" => (float) trim($factura["VAL_EQUI"]),
                "estatus" => 1,
                "creado" => date("Y-m-d H:i:s"),
            );
