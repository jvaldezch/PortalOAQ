...
class OAQ_Imprimir_Proforma extends TCPDF {
    ...
    function __construct($data, $orientation, $unit, $format) {
        ...
        $this->_margins = 26;
        $this->SetMargins($this->_margins, 50, $this->_margins, true);
    ...
    public function Header() {        
        ...
        $this->MultiCell(0, 0, "PROFORMA FACTURA " . $this->_data["trafico"]["numeroFacturaOriginal"], 0, 'C', 1, 0, '', '', true, 0, false, true, 0);
    ...
    public function Footer() {
        ...
        $this->SetLineStyle(array("width" => 1.0, "cap" => "butt", "join" => " 'miter'", "dash" => 0, "color" => array(30, 30, 30)));
        $this->MultiCell(0, 38, "Proforma factura para fines aduanales únicamente", "T", "L", 1, 0, "", "", true, 0, true, true, 8, "B");
    }

    public function Create() {
        $this->AddPage();

        $data = $this->_data["trafico"];
        ...
        $this->Ln(1);
        $this->_generales($this->_data["emisor"], "proveedor");
        $this->Ln(1);
        $this->_generales($this->_data["destinatario"], "destinatario");
        $this->Ln(1);
        ...
        if(isset($this->_data["mercancias"]) && !empty($this->_data["mercancias"])) {
            foreach ($this->_data["mercancias"] as $item) {
                if(($this->getPageHeight() - $this->GetY()) < 200) {
                    $this->AddPage();
                }
    ...
    protected function _generales($data, $title) {
        ...
        if (isset($data["calle"])) {
            $linecount = $this->getNumLines($this->_vdata($data, "calle"), 255);
        } else {
            $linecount = 1;            
        }
        $this->MultiCell(260, 13 * $linecount, $this->_vdata($data, "calle"), "BL", 'L', 1, 0, '', '', true, 0, false, true, 0);
        $this->MultiCell(100, 13 * $linecount, $this->_vdata($data, "numeroExterior"), "BLR", 'L', 1, 0, '', '', true, 0, false, true, 0);
        $this->MultiCell(100, 13 * $linecount, $this->_vdata($data, "numeroInterior"), "BR", 'L', 1, 0, '', '', true, 0, false, true, 0);
        $this->MultiCell(100, 13 * $linecount, $this->_vdata($data, "codigoPostal"), "BR", 'L', 1, 0, '', '', true, 0, false, true, 0);
        ...
        if(isset($data["colonia"])) {
            $linecount = $this->getNumLines($this->_vdata($data, "colonia"), 255);
        } else {
            $linecount = 1;            
        }
        $this->MultiCell(260, 13 * $linecount, $this->_vdata($data, "colonia"), "BL", 'L', 1, 0, '', '', true, 0, false, true, 0);
        $this->MultiCell(300, 13 * $linecount, $this->_vdata($data, "localidad"), "BLR", 'L', 1, 0, '', '', true, 0, false, true, 0);
        $this->Ln();
        ...
        $this->SetFont($this->_font, '', $this->_fontSize);
        $this->SetFillColor(260, 255, 255);
        if (isset($data["entidadFederativa"]) && !is_array($data["entidadFederativa"])) {
            $linecount = $this->getNumLines($this->_vdata($data, "entidadFederativa"), 255);
        } else {
            $linecount = 1;            
        }
        $this->MultiCell(260, 13 * $linecount, $this->_vdata($data, "entidadFederativa"), "BL", 'L', 1, 0, '', '', true, 0, false, true, 0);
        $this->MultiCell(300, 13 * $linecount, $this->_vdata($data, "municipio"), "BLR", 'L', 1, 0, '', '', true, 0, false, true, 0);
        $this->Ln();
        ...
        $this->SetFont($this->_font, '', $this->_fontSize);
        $this->SetFillColor(260, 255, 255);
        $this->MultiCell(0, 0, $mppr->getName($this->_vdata($data, "pais")), "BLR", 'L', 1, 0, '', '', true, 0, false, true, 0);
    }

    protected function _mercancia($data = null) {
        ...
        if (isset($data)) {
            ...
            $this->SetFont($this->_font, '', $this->_fontSize);
            $this->SetFillColor(260, 255, 255);
            $this->MultiCell(0, 0 , "NO HAY MERCANCIAS", "BTLR", 'C', 1, 0, '', '', true, 0, false, true, 0);
        }
    }
    ...
    protected function _certificadoDeOrigen($data) {
        if (isset($data["certificadoOrigen"])) {
            if ($data["certificadoOrigen"] == '1') {
    ...
    protected function _numeroExportador($data) {
        if (isset($data["numeroExportadorAutorizado"])) {
            if ($data["numeroExportadorAutorizado"] != '') {
                return $data["numeroExportadorAutorizado"];
            }
        } else {
            return "";
        }
