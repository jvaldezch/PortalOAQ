...
class OAQ_Xml {
    ...
    function get_firmaElectronica() {
        return $this->_firmaElectronica;
    }
    ...
    function __construct($cove = false, $edoc = false, $downloadEdoc = false) {
        ...
        if ($downloadEdoc == false) {
            ...
            $this->_envelope = $this->_domtree->createElementNS('http://schemas.xmlsoap.org/soap/envelope/', 'soapenv:Envelope');
            $this->_envelope->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:tem', 'http://tempuri.org/');
            $this->_domtree->appendChild($this->_envelope);
        }
    ...
    public function documentoDigitalizado($username, $password, $edocument) {
        $this->_header->appendChild($this->_domtree->createElement("tem:Username", $username));
        $this->_header->appendChild($this->_domtree->createElement("tem:Password", $password));
        $documentoIn = $this->_domtree->createElement("tem:DocumentoIn");
        $documentoIn->appendChild($this->_domtree->createElement("tem:Edocument", $edocument));
        $documentoIn->appendChild($this->_domtree->createElement("tem:IsCertificado", 1));
        $this->_body->appendChild($documentoIn);
    }
    ...
    public function imitarEdocument($data, $hideCredentials = false, $selloDigital = null) {
        try {
            $this->_array = $data;
            if ($hideCredentials === false) {
                $this->_credenciales();
            ...
            $this->_firmaElectronica->appendChild($this->_domtree->createElement("{$namespace}:firma", $selloDigital));
        } catch (Exception $ex) {
            throw new Exception("Zend Exception found on " . __METHOD__ . " : " . $ex->getMessage());
        }
    }
    ...
    protected function _firma($cadena) {
        try {
            ...
            $this->_signature = base64_encode($firma);
    ...
    protected function _generalesCove() {
        ...
        foreach ($keys as $key) {
            if (isset($this->_array["factura"][$key])) {
                if (trim($this->_array["factura"][$key]) !== "") {
        ...
        $factura->appendChild($this->_domtree->createElement("oxml:certificadoOrigen", $this->_array["factura"]["certificadoOrigen"]));
        $factura->appendChild($this->_domtree->createElement("oxml:subdivision", $this->_array["factura"]["subdivision"]));
     ...
     * @throws Exception
     */
    public function saveToDisk($type = null, $name = null) {
        try {
