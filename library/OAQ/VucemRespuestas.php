...
class OAQ_VucemRespuestas {
     ...
     * @param type $xmlstr
     */
    public function analizarRespuestaEdocument($xmlstr) {
        if (!preg_match('/uuid:/i', $xmlstr)) {
            $array = $this->_analizarRespuestaEdocument($this->replace($xmlstr));
        } else {
            $fix = "<S:Envelope>" . $this->stringInsideTags($xmlstr, "S:Envelope") . "</S:Envelope>";
            $array = $this->_analizarRespuestaEdocument($fix);
        }
        return $array;
    }
    ...
    protected function _analizarRespuestaEdocument($xmlstr) {
        $array = $this->xmlstrToArray($this->replace($xmlstr));
        Zend_Debug::dump($array);
        if (isset($array["s:Body"])) {
            if (isset($array["s:Body"]["DocumentoOut"])) {
                $res = $array["s:Body"]["DocumentoOut"];
                if (isset($res["TieneError"]) && (boolean) $res["TieneError"] == true) {
                    $msg["error"] = true;
                    $msg["messages"] = $this->_getMessages($res["Errores"]);
                }
                if (isset($res["TipoDocumento"])) {
                    $msg["tipoDocumento"] = (int) $res["TipoDocumento"];                    
                }
            } else {
                $msg["error"] = true;
            }
            ...
            if (isset($array["Body"]) && isset($array["Body"]["ConsultarEdocumentResponse"]) && isset($array["Body"]["ConsultarEdocumentResponse"]["response"])) {
                ...
                if (isset($res["leyenda"]) && !isset($res["numeroOperacion"])) {
                    if (preg_match('/No existe el n/i', $res["leyenda"])) {
                        $msg["messages"] = $this->_getMessages($res["leyenda"]);
                        $msg["error"] = true;
                    }
                }
                if (isset($res["leyenda"]) && !isset($res["numeroOperacion"])) {
                    if (preg_match('/Firma inv/i', $res["leyenda"])) {
                        $msg["messages"] = $this->_getMessages($res["leyenda"]);
                        $msg["error"] = true;
                    }
                }
                ...
                if (isset($res["faultcode"]) && isset($res["faultstring"])) {
                    if (preg_match('/FailedAuthentication/i', $res["faultcode"])) {
                        $msg["messages"] = $this->_getMessages("No se reconoce el usuario.");
                        $msg["error"] = true;
                    }
                    if (preg_match('/Undeclared namespace prefix/i', $res["faultstring"])) {
                        $msg["messages"] = $this->_getMessages("El XML no esta debidamente conformado.");
                        $msg["error"] = true;
                    }
    ...
    protected function _getMessages($message) {
        if (is_string($message)) {
            return array(utf8_decode($message));
        } elseif (is_array($message)) {
            $msgs = array();
            foreach ($message as $msg) {
                $msgs[] = utf8_decode($msg);
            }
            return $msgs;
        }
        return array();
