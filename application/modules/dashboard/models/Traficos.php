    ...
    public function obtenerTraficos($page, $size, $fecha = null, $rfcCliente = null)
    {
        try {
            ...
            if (APPLICATION_ENV == 'production') {
                $sql->where("t.fechaEta >= ?", date('Y-m-d', strtotime('-15 days', strtotime($fecha))));
            } else {
                $sql->where("t.fechaEta >= ?", date('Y-m-d', strtotime('-95 days', strtotime($fecha))));
            }
            if (isset($size)) {
    ...
    public function total($fecha = null, $rfcCliente = null)
    {
        try {
            $sql = $this->_db_table->select()
                ...
                ->where("estatus <> 4");
            if (APPLICATION_ENV == 'production') {
                $sql->where("fechaEta >= ?", date('Y-m-d', strtotime('-15 days', strtotime($fecha))));
            } else {
                $sql->where("fechaEta >= ?", date('Y-m-d', strtotime('-95 days', strtotime($fecha))));
            }
            if ($rfcCliente) {
                $sql->where("rfcCliente = ?", $rfcCliente);
            }
