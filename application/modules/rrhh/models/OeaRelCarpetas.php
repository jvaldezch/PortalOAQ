...
class Rrhh_Model_OeaRelCarpetas {

    protected $_db_table;

    public function __construct() {
        $this->_db_table = new Rrhh_Model_DbTable_OeaRelCarpetas();
    }
    ...
    public function obtener($directory = null) {
        try {
            $sql = $this->_db_table->select()
                    ...
                    ->from(array("r" => "oea_relcarpetas"), array("id"))
                    ->joinLeft(array("cc" => "oea_carpetas"), "cc.id = r.idCarpeta", array(""))
                    ->joinLeft(array("p" => new Zend_Db_Expr("(SELECT id, carpeta FROM oea_carpetas)")), "p.id = r.idParent", array("carpeta AS previo"))
                    ->joinLeft(array("c" => new Zend_Db_Expr("(SELECT id, carpeta, nombreCarpeta, creada, modificada FROM oea_carpetas)")), "c.id = r.idChild", array("carpeta AS siguiente", "nombreCarpeta", "creada", "modificada", "id AS idCarpeta"))
    ...
    public function obtenerParent($directory = null) {
        try {
            $sql = $this->_db_table->select()
                    ->setIntegrityCheck(false)
                    ->from(array("r" => "oea_relcarpetas"), array(""))
                    ->joinLeft(array("cc" => "oea_carpetas"), "cc.id = r.idCarpeta", array(""))
                    ->joinLeft(array("p" => new Zend_Db_Expr("(SELECT id, carpeta FROM oea_carpetas)")), "p.id = r.idParent", array("carpeta AS previo"))
                    ->group("previo");
            if (isset($directory)) {
    ...
    public function obtenerParentArray($directory = null) {
        try {
            $sql = $this->_db_table->select()
                    ->setIntegrityCheck(false)
                    ->from(array("r" => "oea_relcarpetas"), array(""))
                    ->joinLeft(array("cc" => "oea_carpetas"), "cc.id = r.idCarpeta", array("nombreCarpeta"))
                    ->joinLeft(array("p" => new Zend_Db_Expr("(SELECT id, carpeta FROM oea_carpetas)")), "p.id = r.idParent", array("carpeta AS previo"))
                    ->group("previo");
            if (isset($directory)) {
                $sql->where("cc.carpeta = ?", $directory);
