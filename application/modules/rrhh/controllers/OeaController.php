...
class Rrhh_OeaController extends Zend_Controller_Action {
    ...
    public function indexAction() {
        $this->view->title = $this->_appconfig->getParam("title") . " Archivos OEA (Manager)";
        ...
        $this->view->headScript()
                ...
                ->appendFile("/js/rrhh/oea/index.js?" . time());
        $f = array(
    ...
    public function crearDirectorioAction() {
        try {
            ...
            if ($r->isPost()) {
                ...
                $rels = new Rrhh_Model_OeaRelCarpetas();
                $folders = new Rrhh_Model_OeaCarpetas();
                $folder = realpath(APPLICATION_PATH . "/../public/oea/");
    ...
    public function editarDirectorioAction() {
        try {
            ...
            if ($r->isPost()) {
                ...
                $input = new Zend_Filter_Input($f, $v, $r->getPost());
                if ($input->isValid("id") && $input->isValid("nombre")) {
                    $mapper = new Rrhh_Model_OeaCarpetas();
                    if (($mapper->actualizarNombre($input->id, html_entity_decode($input->nombre)))) {
                        $this->_helper->json(array("success" => true, "nombre" => $input->nombre));
                    }
                }
    ...
    public function subirArchivosAction() {
        try {
            ...
            if ($r->isPost()) {
                ...
                $archivos = new Rrhh_Model_OeaArchivos();
    ...
    public function eliminarDirectorioAction() {
        try {
            ...
            if ($r->isPost()) {
                ...
                if ($input->isValid("id")) {
                    $folder = realpath(APPLICATION_PATH . "/../public/oea/");
                    $r = new Rrhh_Model_OeaRelCarpetas();
                    $c = new Rrhh_Model_OeaCarpetas();
                    ...
                    if (isset($d) && !empty($d)) {
                        ...
                        if (file_exists($f) && $f != realpath(APPLICATION_PATH . "/../public/oea/")) {
                            unlink($f);
                        }
                        $this->_helper->json(array("success" => true));
                    }
