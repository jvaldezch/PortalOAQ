...
filename="db_vtiger_$now".gz
backupfolder="/tmp/bakcuptmp"
nasfolder="/mnt/nas/dbs"
fullpathbackupfile="$backupfolder/$filename"
fullpathnas="$nasfolder/$filename"
logfile="$backupfolder/"backup_log_vtiger_"$(date +'%Y_%m')".txt
echo "mysqldump started at $(date +'%d-%m-%Y %H:%M:%S')" >> "$logfile"
mysqldump --user=root --password=mysql11! --default-character-set=utf8 vtiger | gzip > "$fullpathbackupfile"
